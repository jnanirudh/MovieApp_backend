...
from schemas.movie import HomePage_List, SearchResult, MovieDetail

router = APIRouter(prefix="api/movies", tags= ["Movies"])

@router.get("/homepage", response_model = HomePage_List) # HomePage_List is the DTO defined in movie_schema file
def popular_movies(
    page: int = Query(1, get = 1), # Default page number = 1 and page number must be >= 1 (ge)
    limit: int = Query(20, get = 1, le = 50), # Default movie limit = 20 and limit must be >= 1 (ge) and <= 50 (le)
